...
sshpass -p $PASS scp -P $PORT $PWD/deploying-service-master-host.sh $USR@$IP:/opt;
sshpass -p $PASS scp -P $PORT $PWD/cassandra-performance.sh $USR@$IP:/opt;
echo "Transfer autoload files"
sshpass -p $PASS scp -P $PORT $PWD/autostart-service/spark-master.service $USR@$IP:/etc/systemd/system;
...
sshpass -p $PASS ssh $USR@$IP -p $PORT '/opt/deploying-service-master-host.sh'
